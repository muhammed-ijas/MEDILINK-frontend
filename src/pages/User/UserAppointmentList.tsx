...
const UserAppointmentList = () => {
  ...
  const currentAppointments = appointmentDetails.slice(indexOfFirstAppointment, indexOfLastAppointment);

  const paginate = (pageNumber: number) => setCurrentPage(pageNumber);

  return (
    <div className="container mx-auto mt-8 p-4 min-h-screen">
      <h2 className="text-2xl font-semibold mb-4 text-center">User Appointments</h2>
          ...
          <tbody>
            {currentAppointments.map((appointment) => (
              <tr key={appointment._id} className="border-t">
                <td className="py-2 px-4">{appointment.serviceProvider.name}</td>
                <td className="py-2 px-4">{appointment.doctor.name}</td>
                <td className="py-2 px-4">{appointment.patientName}</td>
                <td className="py-2 px-4">{new Date(appointment.bookingDate).toLocaleDateString()}</td>
                <td className="py-2 px-4">{new Date(appointment.createdAt).toLocaleDateString()}</td>
      ...
      <div className="flex justify-center mt-6">
        <nav>
          <ul className="inline-flex items-center space-x-2">
            {Array.from({ length: Math.ceil(appointmentDetails.length / appointmentsPerPage) }, (_, index) => (
              <li key={index}>
                <button
